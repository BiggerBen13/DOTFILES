...
M = {
	...
	-- autosnippet({ trig = "(\\\\?\\w+)(,\\.|\\.,)", regTrig = true, name = "Vector", trigEngine = "emca" },
	-- 	fmta([[\\vec{<>}]],{
	-- 	f(function(_, snip)
	-- 		return snip.captures[1]
	...
	autosnippet(
		{ trig = "(\\\\?\\w+)(,\\.|\\.,)", wordTrig = false, regTrig = true, name = "Vector", trigEngine = "ecma" },
		fmta([[ \vec{<>} ]], {
			f(function(_, snip)
				return snip.captures[1]
			end)
	...
	autosnippet(
		{ trig = "\\b(,m|m,)", wordTrig = false, regTrig = true, name = "Vector", trigEngine = "ecma" },
		fmta([[ \vec{<>}<> ]], {
			i(1), i(2)
		}),
		{ condition = tex.in_math }),
